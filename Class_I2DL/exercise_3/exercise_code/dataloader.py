...
class FacialKeypointsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        dic_image = np.array([int(item) for item in image_string.split()]).reshape((1, 96, 96))
        
        keypoint_cols = list(self.key_pts_frame.columns)[:-1]
        dic_keypoints = self.key_pts_frame.iloc[idx][keypoint_cols].values.reshape((15, 2)).astype(np.float32)
        
        sample = {'image': dic_image, 'keypoints': dic_keypoints}
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
