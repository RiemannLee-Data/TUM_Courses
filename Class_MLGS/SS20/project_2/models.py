...
class SmoothClassifier(nn.Module):
    ...
    def certify(self, inputs: torch.Tensor, n0: int, num_samples: int, alpha: float, batch_size: int) -> Tuple[int,
        ...
        ##########################################################
        # YOUR CODE HERE
        # determine the lower bound of px*
        # see slide05-95 for reference

        standard_result = self._sample_noise_predictions(inputs, n0, batch_size)
        # note that the index of a torch is still a torch variable!!!
        top_class = standard_result.argmax().item()

        robust_result = self._sample_noise_predictions(inputs, num_samples, batch_size)
        nA = robust_result[top_class].item()
    ...
    def predict(self, inputs: torch.tensor, num_samples: int, alpha: float, batch_size: int) -> int:
        ...
        ##########################################################
        # YOUR CODE HERE
        param1, param2 = class_counts.argsort()[-1].item(), class_counts.argsort()[-2].item()
        # param1 = class_counts.argsort()[-1].item()
        # param2 = class_counts.sum().item()

        # note here the binom_test is less than 0.5, which deviation from binary distribution
        # if binom_test(param1, param1 + param2, p=0.5, alternative="less") < alpha:
        # if binom_test(param1, param1 + param2, p=0.5, alternative="greater") < alpha:
        if binom_test(param1, param1 + param2, p=0.5, alternative="two-sided") < alpha:
            return class_counts.argmax().item()
        else:
            return -1
        ##########################################################
    ...
    def _sample_noise_predictions(self, inputs: torch.tensor, num_samples: int, batch_size: int) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            for it in range(ceil(num_samples / batch_size)):
                ...
                # YOUR CODE HERE
                # _, C, N, N = inputs.size()
                # epsilon = torch.randn((this_batch_size, C, N, N)) * self.sigma
                # inputs_perturb = inputs + epsilon
                # can just easily use forward here
                pert_input = inputs.repeat(this_batch_size, 1, 1, 1)
                logits_perturb = self.forward(pert_input)
                # logits_perturb = self.base_classifier(inputs_perturb)
                num_remaining -= this_batch_size
                # two output here: max_value and max_index
                # tensors used as indices must be long, byte or bool tensors

                _, top_1 = torch.max(logits_perturb, dim=1)

                for predicted_class in top_1:
