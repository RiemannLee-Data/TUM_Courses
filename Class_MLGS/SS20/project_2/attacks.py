...
def fast_gradient_attack(logits: torch.Tensor, x: torch.Tensor, y: torch.Tensor, epsilon: float, norm: str = "2",
    ...
    if norm == "1":
        perturb = sign_data_grad / torch.norm(sign_data_grad, dim=(1, 2, 3), p=1, keepdim=True)
    elif norm == "2":
        perturb = sign_data_grad / torch.norm(sign_data_grad, dim=(1, 2, 3), p=2, keepdim=True)
    else:
        perturb = sign_data_grad / torch.norm(sign_data_grad, dim=(1, 2, 3), p=float('inf'), keepdim=True)
    # Create the perturbed image by adjusting each pixel of the input image
    x_perturb = x + epsilon * perturb

    # Adding clipping to maintain [0,1] range
    x_pert = torch.clamp(x_perturb, 0, 1)
